...
return new class extends Migration
{
    ...
    {
        $this->replaceView(includeBlocked: true);
    }
    ...
    {
        $this->replaceView(includeBlocked: false);
    }

    private function replaceView(bool $includeBlocked): void
    {
        ...
        $leaseBlockedSelect = $includeBlocked ? "                    blocked,\n" : '';
        $matchBlockedSelect = $includeBlocked ? "                    leases.blocked,\n" : '';
        $viewBlockedSelect = $includeBlocked ? "                blocked,\n" : '';
        ...
        DB::statement(<<<SQL
            ...
            WITH leases AS (
                SELECT
                    id,
                    mikrotik_config_id,
                    {$leaseBlockedSelect}comment AS dhcp_name,
            ...
            matches AS (
                SELECT
                    ...
                    {$matchBlockedSelect}leases.dhcp_name,
                    leases.lease_ip,
            ...
            SELECT
                row_number() OVER (
                    ORDER BY
                        ...
                        COALESCE(dhcp_name, queue_name, '')
                ) AS id,
                mikrotik_config_id,
                dhcp_lease_id,
                queue_id,
                {$viewBlockedSelect}dhcp_name,
                lease_ip,
                lease_address,
                ...
                CASE
                    ...
                    WHEN queue_name IS NULL OR btrim(queue_name) = '' THEN 'Queue Belum Ada Nama'
